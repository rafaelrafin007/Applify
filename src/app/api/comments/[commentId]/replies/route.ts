...
const createReplySchema = z.object({
  content: z.string().trim().min(1, "Reply content is required").max(3000, "Reply is too long"),
});
...
function normalizeReplyInput(raw: RawReplyInput) {
  return {
    content: typeof raw.content === "string" ? raw.content : "",
  };
}

async function parseRequestBody(request: Request): Promise<RawReplyInput> {
...
export async function POST(request: Request, context: { params: Promise<{ commentId: string }> }) {
  ...
  try {
    ...
    const parsed = createReplySchema.safeParse(normalizeReplyInput(rawInput));
    ...
    return NextResponse.json({ message: "Reply created successfully", reply }, { status: 201 });
  } catch (error) {
